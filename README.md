...
#### 20210604(금) 작업.
- 오라클일때: localhost:1521/XE 접속URL끝의 XE 서비스 ID명
- 오라클은 사용자명(XE2)이 테이블스페이스명(DB명)입니다. 사용자(XE2)워크스페이스를 추가하면 테이블스페이스(DB)가 새로 만들어 집니다.(구버전은 데스크탑 프로그램 -> 지금은 웹 프로그램 사용자추가/DB(테이블스페이스)추가
- Mysql(마리아DB): localhost:3306/XE URL 끝의 XE 스키마명(DB명)
