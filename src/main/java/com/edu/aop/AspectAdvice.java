...
public class AspectAdvice {
	...
	public Object sessionManager(ProceedingJoinPoint pjp) throws Throwable {
		...
		for(Object object:pjp.getArgs()) {
			if(object instanceof PageVO) { //AOP 실행 메서드 중 매개 변수 PageVO pageVO 객체 판단
				//결과는 게시판, 멤버서비스에 PageVO사용하는 서비스에만 적용 됨. 게시판 생성 관리에는 적용 안됨.
		...
		if(request != null) {//jsp에서 Get,Post 있을때,
			...
			//검색폼이 있는 jsp에서 발생 됨. 결과는 검색폼이 없는 입력/수정/삭제에서는 실행 안됨.
			...
			if(session.getAttribute("session_search_keyword") != null) {
				...
				if(pageVO != null) { //겟,셋 중에 Set할 때 pageVO 널이면 에러 발생 하기 때문에 추가한 코드
