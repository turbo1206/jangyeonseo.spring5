...
public class AdminController {
	...
	public String board_update_form(Model model, @RequestParam("bno")Integer bno, @ModelAttribute("pageVO") PageVO pageVO) throws Exception {
		//첨부파일용 save_file_names, real_file_names 2개 배열 값을 구해서 boardVO 입력이 필요
		...
		int idx=0;
		//향상 된 for문 사용
		for(AttachVO file_name:listAttachVO) { //세로 데이터를 가로 데이터로 변경하는 로직
		...
		return "admin/board/board_update"; //.jsp 생략
	}
