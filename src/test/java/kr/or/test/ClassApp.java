...
/**
 * 이 클래스는 1. 클래스 상속(extends)에 연습, 2. 오브젝트(객체)생성과 new 키워드(예약어) 생성자 메서드 사용 연습, 3.
 * 추상클래스(Abstract) 대한 연습
 * 
 * @author User
 *
 */
//오브젝트생성에 필요한 new 키워드 생성자 메서드 사용한 코딩 클래스(아래)
class Circle {
	...
	// 원의 넓이를 구하는 반환(리턴)값의 크기가 더블형(아래)
...
class Salesman extends Employee {// extends 확장 -> 상속
	public Salesman() { // 클래스명과 똑같은 이름의 메서드가 생성자 함수입니다.
		// 자동으로 생성되는데, 개발자가 커스텀마이징 할 때 만듭니다. (아래)
		mDept = "판매 부서";// 부모클래스의 변수를 상속 받아서 사용 가능.
	}
...
abstract class GraphicObject {
	...
	abstract void draw();// 구현내용이 없고, 메서드 명만 존재. 추상 메서드입니다.
...
class Rectangle extends GraphicObject {
    //사각형 만들기
	@Override //오버라이드 @에노테이션이 있으면, 상속이라는 말.
	void draw() {
		System.out.println("*****");
		System.out.println("*****");
		System.out.println("*****");
...
public class ClassApp {
	...
	public static void main(String[] args) {
		...
		GraphicObject rectangleleObject = new Rectangle();
		traiangleObject.draw();
		rectangleleObject.draw();
