...
class MemberVO {
	...
	//하나씩 출력하기 힘들 때 자동으로 출력하는 멤버 변수 값을 출력하는 명령 toString생성
	@Override
	public String toString() {
		return "MemberVO 출력 [name=" + name + ", age=" + age + ", phoneNum=" + phoneNum + "]";
	}
	//위 프라이빗 멤버 변수를 입출력 구현한 메서드를 만듭니다.
...
public class Step2 {
	...
	public static void main(String[] args) {
		...
		//memberVO클래스를 배열 객체(String[]처럼) 생성했음(아래)
		MemberVO[] members = new MemberVO[3];
		members[0] = memberVO;
		members[1] = memberVO2;
		members[2] = memberVO3;
		//출력도 메서드를 바로 호출하지 않고, 외부 클래스에서 객체로 만들어서 메서드를 호출
		MemberService memberService = new MemberService();
		memberService.printMember(members);
		//객체로 만들면, 호출(runtime)시 메모리에 로딩됩니다. -> 실행 끝나면, 반환.
		//외부클래스는 아래처럼 직접 접근해서 메서드나 변수를 사용 할 수 없습니다.
		//외부클래스로 접근하려면, 객체(실행가능한=메모리로딩)로 만들어야 가능함.
		//static 예약어는 컴파일시 메모리에 로딩(객체가됨)이 되게 명시
		//매번 static으로 만들면 메모리가 가득차서 프로그램 실행이 느려지거나 멈춤.
        //MemberService.printMember(members);
...
class MemberService {
	...
	// 멤버서비스클래스에서 퍼블릭 접근 제어로 회원정보를 출력하는 메서드 생성
}
